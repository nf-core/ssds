...
## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/ssds/usage](https://nf-co.re/ssds/usage)
...
You will need to create a samplesheet with information about the samples you would like to analyse before running the pipeline. Use this parameter to specify its location. The format is a comma-separated file with 3 columns, and a header row as shown in the examples below.
...
There is a strict requirement for the first 3 columns to match those defined in the table below. Remember, SSDS samples MUST be run in paired-end mode as otherwise, key molecular features used to identify ssDNA cannot be identified. 
...
Currently, the pipeline treats each entry row of the samplesheet as an independent sample. FASTQ files from resequencing should be pooled prior to running. 
...
```console
nextflow run nf-core/ssds --input samplesheet.csv --genome mm19 -profile singularity
```
...
First, go to the [nf-core/ssds releases page](https://github.com/nf-core/ssds/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
